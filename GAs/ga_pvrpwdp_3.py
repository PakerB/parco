...
class DecodedSolution:
    routes: list[list[int]]  # Thay states bằng routes thuần túy
...
class BasicPVRPWDPSolver:
    ...
    def __init__(self, env: PVRPWDPVEnv, td_raw: torch.Tensor, td_reset: torch.Tensor):
        ...
        # Clamp tránh chia 0 khi có padded agents có speed=0
        ...
        # Pre-compute distance matrix (Siêu tối ưu tốc độ so với np.linalg.norm mỗi lần gọi)
    ...
    def decode(self, chromosome: np.ndarray) -> DecodedSolution:
        ...
        # Thay vì dùng class, khởi tạo vector 1D cho toàn bộ trạng thái xe (Cực nhanh)
            ...
            for v in range(self.num_agents):
                ...
                if u_cap[v] + demand <= capacity + 1e-9:
                    ...
                    if service_time <= float(latest) + 1e-9:
                        wait_mid = max(float(earliest) - arrival, 0.0) if c_node[v] != v else 0.0
                        ...
                        if return_time <= new_deadline + 1e-9 and new_u_end + back_time <= endurance + 1e-9:
                            rank = (return_time, travel_dist, service_time)
                            if rank < best_rank:
                                best_rank = rank
                                best_vehicle = v
                                best_update = (cust_idx, service_time, c_length[v] + travel_dist, 
                                               u_cap[v] + demand, new_u_end, new_deadline, False)
                ...
                if c_node[v] != v:
                    ...
                    c_time_reset = c_time[v] + back_dist / speed
                    ...
                    if demand <= capacity + 1e-9:
                        ...
                        if service_time_new <= float(latest) + 1e-9:
                            ...
                            new_deadline_new = min(self.max_time, service_time_new + wait_limit)

                            if return_time_new <= new_deadline_new + 1e-9 and new_u_end_new + back_time_new <= endurance + 1e-9:
                                rank_new = (return_time_new, travel_dist_new, service_time_new)
                                if rank_new < best_rank:
                                    best_rank = rank_new
                                    best_vehicle = v
                                    best_update = (cust_idx, service_time_new, c_length_reset + travel_dist_new, 
                                                   demand, new_u_end_new, new_deadline_new, True)

            # Cập nhật trạng thái sau khi đã tìm được best choice
            if best_vehicle != -1 and best_update is not None:
                v = best_vehicle
                n_c_node, n_c_time, n_c_len, n_u_cap, n_u_end, n_t_dead, is_reset = best_update
                
                if is_reset:
                    # Ghi nhận chặng về kho
                    c_time[v] += float(self.dist_matrix[c_node[v], v]) / self.agents_speed[v]
                    routes[v].append(v)
        ...
        for v in range(self.num_agents):
            if c_node[v] != v:
                ...
                c_time[v] += back_dist / self.agents_speed[v]
                c_node[v] = v
                u_cap[v] = 0.0
                u_end[v] = 0.0
        ...
        travel_part = float(np.sum(c_length * self.travel_price))
        ...
        score = self.objective_cost_from_parts(self.env.target, len(unserved), total_cost, makespan)
        ...
        self._score_cache[key] = (score, solution)
        return solution

    def evaluate(self, chromosome: np.ndarray) -> float:
        return self.decode(chromosome).score

    def objective_cost_from_parts(self, target: str, unserved_count: int, total_cost: float, makespan: float) -> float:
        if target == "mincost":
    ...
    def objective_cost(self, solution: DecodedSolution) -> float:
        return self.objective_cost_from_parts(self.env.target, len(solution.unserved_customers), solution.total_cost, solution.makespan)
    ...
    def chromosome_seeds(self) -> list[np.ndarray]:
        ...
        depot_dist = np.argsort([float(self.dist_matrix[0, int(env_idx)]) for env_idx in self.customer_env_indices])
    ...
    def local_search(self, chromosome: np.ndarray, rng: random.Random, max_iters: int = 20, neighborhood_trials: int = 12) -> np.ndarray:
    ...
    def tournament_select(self, population: list[np.ndarray], fitness: list[float], tournament_size: int, rng: random.Random) -> np.ndarray:
        tournament_size = min(tournament_size, len(population))
        idxs = rng.sample(range(len(population)), tournament_size)
        best_idx = min(idxs, key=lambda idx: fitness[idx])
        return population[best_idx]

    def run(self, population_size: int = 80, generations: int = 200, mutation_rate: float = 0.2, elite_size: int = 4, tournament_size: int = 5, cull_ratio: float = 0.5, immigrant_ratio: float = 0.05, local_search_rate: float = 0.2, local_search_elites: int = 2, local_search_iters: int = 20, seed: int = 42, verbose: bool = True, progress_label: str | None = None, progress_every: int = 10) -> DecodedSolution:
        ...
        for generation in range(generations):
            ...
            if verbose and (generation == 0 or (generation + 1) % max(1, progress_every) == 0 or generation == generations - 1):
                prefix = f"[{progress_label}] " if progress_label else "[GA] "
                print(f"{prefix}{format_progress(generation + 1, generations, run_start_time, width=20)} gen={generation + 1:03d} best_score={decoded[0].score:.4f} unserved={len(decoded[0].unserved_customers)} makespan={decoded[0].makespan:.4f} distance={decoded[0].total_distance:.4f}")
    ...
    def print_solution(self, solution: DecodedSolution) -> None:
        ...
        for v_id, route in enumerate(solution.routes):
            ...
            print(f"  vehicle {v_id}: {' -> '.join(display_route) if display_route else f'D{v_id}'}")

# --- Helper Methods ---

def load_instance_from_td(td_loaded: torch.Tensor, batch_idx: int, target: str) -> tuple[PVRPWDPVEnv, torch.Tensor, torch.Tensor]:
    ...
    # Strip virtual padding (num_real_nodes, num_real_agents) nếu data có padding ảo,
    # nếu không các agent padded speed=0 sẽ làm max_time=NaN và GA không insert được.
...
def summarize_results(results: list[InstanceResult], target: str) -> dict[str, float]:
    count = len(results)
    if count == 0: return {"count": 0.0, "avg_objective_cost": math.nan, "avg_score": math.nan, "avg_unserved": math.nan, "avg_makespan": math.nan, "avg_distance": math.nan, "feasible_rate": math.nan}
    return {
        "count": float(count),
...
def save_results_csv(output_path: Path, results: list[InstanceResult]) -> None:
    ...
    with output_path.open("w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=["target", "batch_idx", "score", "objective_cost", "objective_reward", "raw_env_reward", "unserved", "visited_customers", "done", "makespan", "total_distance", "total_cost"])
        writer.writeheader()
        for item in results: writer.writerow(asdict(item))
...
# --- Chuyển hàm này ra module-level để có thể truyền vào ProcessPoolExecutor ---
def worker_run_single_instance(npz_path: str, 
                               batch_idx: int, 
                               target: str, 
                               population: int, 
                               generations: int, 
                               ...
                               generation_progress_every: int) -> InstanceResult:
    ...
    best = solver.run(
        population_size=population, generations=generations, mutation_rate=mutation_rate,
        elite_size=elite_size, tournament_size=tournament_size, cull_ratio=cull_ratio,
        immigrant_ratio=immigrant_ratio, local_search_rate=local_search_rate,
        local_search_elites=local_search_elites, local_search_iters=local_search_iters,
        seed=seed, verbose=show_generation_progress,
        progress_label=f"{target}|b={batch_idx:03d}", # Label ngắn gọn hơn
    ...
    return InstanceResult(
        target=target, batch_idx=batch_idx, score=float(best.score), objective_cost=float(env_eval["objective_cost"]),
        objective_reward=float(env_eval["objective_reward"]), raw_env_reward=float(env_eval["raw_env_reward"]),
        unserved=len(best.unserved_customers), visited_customers=int(env_eval["visited_customers"]),
        done=int(env_eval["done"]), makespan=float(best.makespan), total_distance=float(best.total_distance),
        total_cost=float(best.total_cost),
    )


def run_target_over_dataset(npz_path: str, target: str, population: int, generations: int, mutation_rate: float, elite_size: int, tournament_size: int, cull_ratio: float, immigrant_ratio: float, local_search_rate: float, local_search_elites: int, local_search_iters: int, seed: int, output_dir: str | None, batch_idx: int | None, num_batches: int | None, show_routes: bool, show_generation_progress: bool, generation_progress_every: int, max_workers: int) -> list[InstanceResult]:
    td_loaded = load_npz_to_tensordict(npz_path)
    ...
    print(f"\n=== Running target={target} on {len(batch_indices)}/{total_batches} instances (Workers: {max_workers}) ===")
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for idx in batch_indices:
            ...
            future = executor.submit(
                ...
                npz_path, idx, target, population, generations, mutation_rate,
                elite_size, tournament_size, cull_ratio, immigrant_ratio,
                local_search_rate, local_search_elites, local_search_iters,
                instance_seed, show_routes and len(batch_indices) == 1,
                show_generation_progress,   # Truyền tham số xuống worker
                generation_progress_every   # Truyền tham số xuống worker
            )
            futures[future] = idx

        for order_idx, future in enumerate(concurrent.futures.as_completed(futures), start=1):
            idx = futures[future]
            try:
                result = future.result()
                results.append(result)
                print(f"[{target}] {format_progress(order_idx, len(batch_indices), start_time)} batch={idx:03d} objective_cost={result.objective_cost:.4f} score={result.score:.4f} unserved={result.unserved} makespan={result.makespan:.4f} done={result.done}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Optimized GA baseline for PVRPWDP (Vectorized + Multi-core).")
    ...
    parser.add_argument("--num-batches", type=int, default=20, help="Chỉ chạy N batch đầu tiên của file NPZ.")
    ...
    parser.add_argument("--max-workers", type=int, default=5, help="Số nhân CPU sử dụng để chạy các batch song song.") # Tham số mới
...
def main() -> None:
    ...
    for target in targets:
        all_results[target] = run_target_over_dataset(
            npz_path=args.npz, target=target, population=args.population, generations=args.generations,
            mutation_rate=args.mutation_rate, elite_size=args.elite_size, tournament_size=args.tournament_size,
            cull_ratio=args.cull_ratio, immigrant_ratio=args.immigrant_ratio, local_search_rate=args.local_search_rate,
            local_search_elites=args.local_search_elites, local_search_iters=args.local_search_iters, seed=args.seed,
            output_dir=args.output_dir, batch_idx=args.batch_idx, num_batches=args.num_batches, show_routes=args.show_routes,
            show_generation_progress=args.show_generation_progress, generation_progress_every=args.generation_progress_every,
            max_workers=args.max_workers, # Gọi đa nhân
        )
...
# Đảm bảo luôn có lệnh python để chạy script này
# python ga_pvrpwdp.py --npz D:\k0d3\DATN\parco\test_data\test.npz --target makespan --population 80 --generations 100 --mutation-rate 0.2 --elite-size 4 --tournament-size 5 --cull-ratio 0.5 --immigrant-ratio 0.05 --local-search-rate 0.2 --local-search-elites 2 --local-search-iters 20 --seed 42 --output-dir /path/to/output --show-routes --show-generation-progress --generation-progress-every 10 --max-workers 5