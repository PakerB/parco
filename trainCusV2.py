...
class CustomPARCORLModule(PARCORLModule):
    def __init__(self, env, policy, train_batch_size=128, val_batch_size=256, test_batch_size=256, **kwargs):
    ...
    def train_dataloader(self):
        current_epoch = self.trainer.current_epoch if self.trainer else self.env.current_epoch
        self.env.current_epoch = current_epoch
        print(f"📊 Đang nạp dữ liệu cho Epoch {current_epoch:02d}...")
        
        # 1. Dọn sạch rác của Epoch cũ để cứu RAM
        import gc
        ...
        # 2. TẠO LẠI DATASET MỚI CHO EPOCH HIỆN TẠI (Dòng quan trọng nhất!)
        self.train_dataset = self.env.dataset(self.train_batch_size)
...
if __name__ == '__main__':
    ...
    trainer = RL4COTrainer(
        ...
        logger=csv_logger,               
        callbacks=[checkpoint_callback],  # Thêm callback
